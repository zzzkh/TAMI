...
if __name__ == "__main__":
    ...
    if args.input_noise_std > 0:
        noise_rng = np.random.default_rng(args.input_noise_seed)
        node_noise = noise_rng.normal(loc=0.0, scale=args.input_noise_std, size=node_raw_features.shape).astype(node_raw_features.dtype, copy=False)
        edge_noise = noise_rng.normal(loc=0.0, scale=args.input_noise_std, size=edge_raw_features.shape).astype(edge_raw_features.dtype, copy=False)
        node_raw_features = node_raw_features + node_noise
        edge_raw_features = edge_raw_features + edge_noise
