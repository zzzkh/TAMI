...
class SnapshotGlobalContextEncoder(nn.Module):
    ...
    def forward(self, src_emb: torch.Tensor, dst_emb: torch.Tensor):
        edge_feat = torch.cat(
            [src_emb, dst_emb, src_emb * dst_emb, torch.abs(src_emb - dst_emb)],
            dim=-1,
        )
        edge_repr = self.edge_mlp(edge_feat)
        snapshot_ctx = edge_repr.mean(dim=0, keepdim=True)
        return snapshot_ctx, edge_repr


class SnapshotFusionGate(nn.Module):
    def __init__(self, hist_dim: int, ctx_dim: int):
        super().__init__()
        self.ctx_proj = nn.Linear(ctx_dim, hist_dim)
        self.gate = nn.Linear(hist_dim * 2, hist_dim)

    def forward(self, hist_pair_emb: torch.Tensor, snapshot_ctx: torch.Tensor):
        ctx = self.ctx_proj(snapshot_ctx)
        if ctx.size(0) == 1:
            ctx = ctx.expand(hist_pair_emb.size(0), -1)
        gate = torch.sigmoid(self.gate(torch.cat([hist_pair_emb, ctx], dim=-1)))
        fused = gate * hist_pair_emb + (1 - gate) * ctx
        return fused, gate


class HistoricalDecoder(nn.Module):

    def __init__(self, input_dim1: int, input_dim2: int, hidden_dim: int, output_dim: int, device='cpu', gamma=0.9, dropout=0.1):
        """
        ...
        self.snapshot_context_encoder = SnapshotGlobalContextEncoder(node_dim=input_dim1, hidden_dim=hidden_dim, dropout=dropout)
        self.snapshot_fusion_gate = SnapshotFusionGate(hist_dim=hidden_dim, ctx_dim=hidden_dim)
        self._pending_keys = []
        self._pending_hist_emb = []

    def _build_keys(self, src_ids, dst_ids):
        if torch.is_tensor(src_ids):
            src_ids = src_ids.detach().cpu().numpy()
        if torch.is_tensor(dst_ids):
            dst_ids = dst_ids.detach().cpu().numpy()
        return list(zip(np.asarray(src_ids).tolist(), np.asarray(dst_ids).tolist()))

    def score_edges(self, src_ids, dst_ids, src_emb: torch.Tensor, dst_emb: torch.Tensor, snapshot_ctx: torch.Tensor = None):
        """
        Score edges without mutating TRC memory.
        Returns logits and the cached historical embedding update.
        """
        x = torch.cat([src_emb, dst_emb], dim=1)
        h_current = self.act(self.fc1(x))
        if snapshot_ctx is not None:
            h_current, _ = self.snapshot_fusion_gate(h_current, snapshot_ctx)

        keys = self._build_keys(src_ids, dst_ids)
        ...
        logits = self.fc2(torch.cat((h_current, h_historical), dim=1))
        return logits, updated_weighted_hist_emb

    def cache_snapshot_updates(self, src_ids, dst_ids, fused_edge_emb: torch.Tensor):
        keys = self._build_keys(src_ids, dst_ids)
        self._pending_keys.extend(keys)
        self._pending_hist_emb.append(fused_edge_emb.detach().cpu())

    def commit_snapshot_updates(self):
        if len(self._pending_keys) == 0:
            return
        pending_hist_emb = torch.cat(self._pending_hist_emb, dim=0)
        self.historical_interaction_memory.update_memories(self._pending_keys, pending_hist_emb)
        self.clear_pending()
    ...
    def forward(self, src_ids, dst_ids, src_emb: torch.Tensor, dst_emb: torch.Tensor, update_memories=False, snapshot_ctx: torch.Tensor = None):
        """
        ...
        """
        logits, fused_edge_emb = self.score_edges(src_ids=src_ids, dst_ids=dst_ids, src_emb=src_emb, dst_emb=dst_emb, snapshot_ctx=snapshot_ctx)
        if update_memories:
            self.cache_snapshot_updates(src_ids=src_ids, dst_ids=dst_ids, fused_edge_emb=fused_edge_emb)
            self.commit_snapshot_updates()
        return logits
...
class TRCMemory(nn.Module):
    ...
    def reload_memory_bank(self, bank):
        """
        Alias kept for compatibility with existing training/evaluation code.
        """
        self.load_memory_bank(bank)
