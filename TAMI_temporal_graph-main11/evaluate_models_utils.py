...
def compute_mrr(pos_scores: torch.Tensor, neg_scores: torch.Tensor):
    """
    Compute MRR from positive scores [B] and negative scores [B, K].
    """
    ranks = 1 + (neg_scores >= pos_scores[:, None]).sum(dim=1)
    return float((1.0 / ranks.float()).mean().item())


def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   evaluate_neg_edge_sampler: NegativeEdgeSampler, evaluate_data: Data, loss_func: nn.Module,
                                   num_neighbors: int = 20, time_gap: int = 2000, number_of_negative_edges: int = 1,
                                   time_mode: str = 'continuous', same_time_mode: str = 'exclude',
                                   snapshot_update_mode: str = 'deferred', max_snapshot_batch_size: int = -1, device=None):
    """
    ...
    if model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer', 'SADyGFormer']:
        model[0].set_neighbor_sampler(neighbor_sampler)

    model.eval()

    def _compute_embeddings(src_node_ids, dst_node_ids, node_interact_times, edge_ids, positive_edges: bool):
        if model_name in ['TGAT', 'CAWN', 'TCL']:
            ...
            return model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=src_node_ids,
                                                                     dst_node_ids=dst_node_ids,
                                                                     node_interact_times=node_interact_times,
                                                                     edge_ids=edge_ids if positive_edges else None,
                                                                     edges_are_positive=positive_edges,
                                                                     num_neighbors=num_neighbors)
            ...
            return model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=src_node_ids,
                                                                     ...
                                                                     num_neighbors=num_neighbors,
                                                                     time_gap=time_gap)
        elif model_name in ['DyGFormer', 'SADyGFormer']:
            return model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=src_node_ids,
                                                                     dst_node_ids=dst_node_ids,
                                                                     node_interact_times=node_interact_times,
                                                                     edge_ids=edge_ids if positive_edges else None,
                                                                     same_time_mode=same_time_mode)
        else:
            raise ValueError(f"Wrong value for model_name {model_name}!")

    with torch.no_grad():
        evaluate_losses, evaluate_metrics = [], []
        num_negative_edges = max(int(number_of_negative_edges), 1)

        if time_mode == 'snapshot' and model_name in ['DyGFormer', 'SADyGFormer']:
            snapshot_batches = build_snapshot_batches(interact_times=evaluate_data.node_interact_times,
                                                      max_snapshot_batch_size=max_snapshot_batch_size)
            evaluate_snapshot_tqdm = tqdm(snapshot_batches, ncols=120)
            for snapshot_idx, snapshot in enumerate(evaluate_snapshot_tqdm):
                full_indices = snapshot['full_indices']
                chunks = snapshot['chunks']

                full_src_node_ids = evaluate_data.src_node_ids[full_indices]
                full_dst_node_ids = evaluate_data.dst_node_ids[full_indices]
                full_node_interact_times = evaluate_data.node_interact_times[full_indices]
                full_edge_ids = evaluate_data.edge_ids[full_indices]
                ...
                for chunk_indices in chunks:
                    batch_src_node_ids = evaluate_data.src_node_ids[chunk_indices]
                    batch_dst_node_ids = evaluate_data.dst_node_ids[chunk_indices]
                    batch_node_interact_times = evaluate_data.node_interact_times[chunk_indices]
                    batch_edge_ids = evaluate_data.edge_ids[chunk_indices]
                    ...
                    pos_scores = pos_logits.squeeze(dim=-1).sigmoid()

                    neg_src_2d, neg_dst_2d = evaluate_neg_edge_sampler.sample_multiple_negative(
                        size=num_negative_edges,
                    ...
                    neg_time_flat = np.repeat(batch_node_interact_times, num_negative_edges)
                    ...
                    neg_scores = neg_logits.squeeze(dim=-1).sigmoid().reshape(len(batch_src_node_ids), num_negative_edges)

                    predicts = torch.cat([pos_scores, neg_scores.reshape(-1)], dim=0)
                    labels = torch.cat([torch.ones_like(pos_scores), torch.zeros_like(neg_scores.reshape(-1))], dim=0)
                    loss = loss_func(input=predicts, target=labels)
                    evaluate_losses.append(loss.item())
                    snapshot_losses.append(loss.item())

                    metric = get_link_prediction_metrics(predicts=predicts, labels=labels)
                    metric['mrr'] = compute_mrr(pos_scores, neg_scores)
                    evaluate_metrics.append(metric)

                    model[1].cache_snapshot_updates(batch_src_node_ids, batch_dst_node_ids, pos_fused)
                    if snapshot_update_mode == 'immediate':
                        model[1].commit_snapshot_updates()

                if snapshot_update_mode == 'deferred':
                    model[1].commit_snapshot_updates()
                avg_snapshot_loss = float(np.mean(snapshot_losses)) if len(snapshot_losses) > 0 else 0.0
                evaluate_snapshot_tqdm.set_description(
                    f'evaluate snapshot {snapshot_idx + 1}, evaluate loss: {avg_snapshot_loss:.4f}'
                )
        else:
            ...
            for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
                ...
                if model_name in ['JODIE', 'DyRep', 'TGN']:
                    neg_src_node_embeddings, neg_dst_node_embeddings = _compute_embeddings(
                        src_node_ids=neg_src_flat,
                        dst_node_ids=neg_dst_flat,
                        node_interact_times=neg_time_flat,
                        edge_ids=None,
                    ...
                    batch_src_node_embeddings, batch_dst_node_embeddings = _compute_embeddings(
                        src_node_ids=batch_src_node_ids,
                        dst_node_ids=batch_dst_node_ids,
                        node_interact_times=batch_node_interact_times,
                        edge_ids=batch_edge_ids,
                ...
                ).squeeze(dim=-1).sigmoid().reshape(len(batch_src_node_ids), num_negative_edges)
