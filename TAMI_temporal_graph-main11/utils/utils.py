...
class NeighborSampler:
    ...
    def __init__(self, adj_list: list, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
        ...
        for node_idx, per_node_neighbors in enumerate(adj_list):
            ...
            sorted_per_node_neighbors = sorted(per_node_neighbors, key=lambda x: (x[2], x[1]))
    ...
    def find_neighbors_before(self, node_id: int, interact_time: float, interact_edge_id: int = None,
                              same_time_mode: str = 'exclude', return_sampled_probabilities: bool = False):
        """
        ...
        """
        # For same-time handling:
        # exclude/snapshot_context -> keep strictly previous interactions (< interact_time)
        # prefix -> additionally keep same-timestamp interactions with smaller edge_id.
        times = self.nodes_neighbor_times[node_id]
        edge_ids = self.nodes_edge_ids[node_id]
        left = np.searchsorted(times, interact_time, side='left')

        if same_time_mode in ['exclude', 'snapshot_context']:
            valid_idx = np.arange(left)
        elif same_time_mode == 'prefix':
            right = np.searchsorted(times, interact_time, side='right')
            same_idx = np.arange(left, right)
            if interact_edge_id is None:
                valid_idx = np.arange(left)
            else:
                same_mask = edge_ids[same_idx] < interact_edge_id
                valid_idx = np.concatenate([np.arange(left), same_idx[same_mask]])
        else:
            raise ValueError(f'Not implemented error for same_time_mode {same_time_mode}!')

        if return_sampled_probabilities:
            return self.nodes_neighbor_ids[node_id][valid_idx], self.nodes_edge_ids[node_id][valid_idx], self.nodes_neighbor_times[node_id][valid_idx], \
                   self.nodes_neighbor_sampled_probabilities[node_id][valid_idx]
        else:
            return self.nodes_neighbor_ids[node_id][valid_idx], self.nodes_edge_ids[node_id][valid_idx], self.nodes_neighbor_times[node_id][valid_idx], None
    ...
    def get_all_first_hop_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
                                    edge_ids: np.ndarray = None, same_time_mode: str = 'exclude'):
        """
        ...
        # get the temporal neighbors at the first hop
        for idx, (node_id, node_interact_time) in enumerate(zip(node_ids, node_interact_times)):
            interact_edge_id = None if edge_ids is None else edge_ids[idx]
            # find neighbors that interacted with node_id before time node_interact_time
            node_neighbor_ids, node_edge_ids, node_neighbor_times, _ = self.find_neighbors_before(node_id=node_id,
                                                                                                  ...
                                                                                                  interact_edge_id=interact_edge_id,
...
class NegativeEdgeSampler(object):
    ...
    def sample_multiple_negative(self, size: int, batch_src_node_ids: np.ndarray = None, batch_dst_node_ids: np.ndarray = None,
        ...
        :return:
        """
        num_negative_edges = max(size, 1)

        if batch_src_node_ids is None:
            # Fallback for legacy callers that do not pass a batch.
            return self.random_sample_multiple_negative(size=num_negative_edges, batch_src_node_ids=None)

        if self.negative_sample_strategy == 'random':
            return self.random_sample_multiple_negative(size=num_negative_edges, batch_src_node_ids=batch_src_node_ids)
        elif self.negative_sample_strategy in ['historical', 'inductive']:
            if batch_dst_node_ids is None:
                raise ValueError('batch_dst_node_ids should not be None when sampling historical/inductive negatives.')

            negative_dst_node_ids_list = []
            for _ in range(num_negative_edges):
                _, sampled_negative_dst_node_ids = self.sample(size=len(batch_src_node_ids),
                                                               batch_src_node_ids=batch_src_node_ids,
                                                               batch_dst_node_ids=batch_dst_node_ids,
                                                               current_batch_start_time=current_batch_start_time,
                                                               current_batch_end_time=current_batch_end_time)
                negative_dst_node_ids_list.append(sampled_negative_dst_node_ids)

            negative_dst_node_ids = np.stack(negative_dst_node_ids_list, axis=1).astype(np.longlong)
            negative_src_node_ids = np.repeat(batch_src_node_ids[:, np.newaxis], num_negative_edges, axis=1).astype(np.longlong)
            return negative_src_node_ids, negative_dst_node_ids
        else:
            raise ValueError(f'Not implemented error for negative_sample_strategy {self.negative_sample_strategy}!')
    
    def random_sample_multiple_negative(self, size: int, batch_src_node_ids: np.ndarray = None):
        """
        random sampling strategy, which is used by previous works
        :param size: int, number of sampled negative edges
        :return:
        ...
        if self.seed is None:
            random_sample_edge_dst_node_indices = np.random.randint(0, len(self.unique_dst_node_ids), (batch_size, size))
        else:
            random_sample_edge_dst_node_indices = self.random_state.randint(0, len(self.unique_dst_node_ids), (batch_size, size))

        negative_dst_node_ids = self.unique_dst_node_ids[random_sample_edge_dst_node_indices].astype(np.longlong)
        negative_src_node_ids = np.repeat(batch_src_node_ids[:, np.newaxis], size, axis=1).astype(np.longlong)
        return negative_src_node_ids, negative_dst_node_ids
