...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--model_name', type=str, default='DyGFormer', help='name of the model, note that EdgeBank is only applicable for evaluation',
                        choices=['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'EdgeBank', 'TCL', 'GraphMixer', 'DyGFormer', 'SADyGFormer'])
    ...
    parser.add_argument('--time_mode', type=str, default='continuous', choices=['continuous', 'snapshot'])
    parser.add_argument('--same_time_mode', type=str, default='exclude', choices=['exclude', 'prefix', 'snapshot_context'])
    parser.add_argument('--snapshot_update_mode', type=str, default='deferred', choices=['immediate', 'deferred'])
    parser.add_argument('--snapshot_context_mode', type=str, default='none', choices=['none', 'global', 'node'])
    parser.add_argument('--train_negative_sample_strategy', type=str, default='random', choices=['random', 'historical', 'inductive'])
    parser.add_argument('--eval_negative_sample_strategy', type=str, default='historical', choices=['random', 'historical', 'inductive'])
    parser.add_argument('--train_num_neg_edges', type=int, default=1)
    parser.add_argument('--eval_num_neg_edges', type=int, default=20)
    parser.add_argument('--max_snapshot_batch_size', type=int, default=-1)
    ...
    # Keep explicit CLI overrides for selected args when loading best configs.
    cli_eval_num_neg_edges = args.eval_num_neg_edges
    cli_max_snapshot_batch_size = args.max_snapshot_batch_size
    ...
    if '--eval_num_neg_edges' in sys.argv:
        args.eval_num_neg_edges = cli_eval_num_neg_edges
    if '--max_snapshot_batch_size' in sys.argv:
        args.max_snapshot_batch_size = cli_max_snapshot_batch_size

    # Backward-compatible aliases: old flags can still control both train/eval behavior.
    if '--negative_sample_strategy' in sys.argv:
        args.train_negative_sample_strategy = args.negative_sample_strategy
        args.eval_negative_sample_strategy = args.negative_sample_strategy
    if '--num_neg_edges' in sys.argv:
        args.train_num_neg_edges = args.num_neg_edges
        args.eval_num_neg_edges = args.num_neg_edges

    args.negative_sample_strategy = args.eval_negative_sample_strategy if is_evaluation else args.train_negative_sample_strategy
    args.num_neg_edges = args.eval_num_neg_edges if is_evaluation else args.train_num_neg_edges
...
def load_link_prediction_best_configs(args: argparse.Namespace):
    ...
    if args.model_name == 'TGAT':
        ...
        args.patch_size = 4
        args.dropout = 0.2
        args.time_mode = 'snapshot'
        args.same_time_mode = 'snapshot_context'
        args.snapshot_update_mode = 'deferred'
        args.snapshot_context_mode = 'global'
        args.train_negative_sample_strategy = 'historical'
        args.eval_negative_sample_strategy = 'historical'
        args.eval_num_neg_edges = 20
        args.negative_sample_strategy = args.eval_negative_sample_strategy
        args.num_neg_edges = args.eval_num_neg_edges
    else:
