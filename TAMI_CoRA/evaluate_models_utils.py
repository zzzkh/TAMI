    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            for i_dx in range(number_of_negative_edges):
                ...
                negative_probabilities = model[1](src_ids = batch_neg_src_node_ids,
                ...
                negative_probs.append(negative_probabilities)

            # positive_probabilities = model[1](input_1=batch_src_node_embeddings, input_2=batch_dst_node_embeddings).squeeze(dim=-1).sigmoid()
            positive_probabilities = model[1](src_ids = batch_src_node_ids, 
            ...
            negative_probs = torch.cat(negative_probs, dim=0)
            predicts = torch.cat([positive_probabilities, negative_probs], dim=0)
            labels = torch.cat([torch.ones_like(positive_probabilities), torch.zeros_like(negative_probs)], dim=0)
