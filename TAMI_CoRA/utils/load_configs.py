...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--decoder_variant', type=str, default='lha', choices=['lha', 'cora_lha'],
                        help='decoder variant used after the temporal backbone.')
    parser.add_argument('--uv_cache_path', type=str, default=None,
                        help='path to the precomputed UV projection cache.')
    parser.add_argument('--cora_num_slots', type=int, default=8, help='number of latent coalition slots in CoRA.')
    parser.add_argument('--cora_profile_rank', type=int, default=16, help='low-rank profile dimension used by the UV projection cache.')
    parser.add_argument('--cora_regime_dim', type=int, default=32, help='hidden dimension of the regime encoder.')
    parser.add_argument('--cora_adapter_hidden_dim', type=int, default=172, help='hidden dimension inside the CoRA adapter.')
    parser.add_argument('--cora_symmetric_pairs', type=int, default=1, choices=[0, 1],
                        help='whether to canonicalize pair keys for CoRA memory and cache lookup.')
    parser.add_argument('--cora_enable_coalition', type=int, default=1, choices=[0, 1],
                        help='whether to enable the coalition branch in CoRA.')
    parser.add_argument('--cora_enable_regime', type=int, default=1, choices=[0, 1],
                        help='whether to enable the regime branch in CoRA.')
    parser.add_argument('--freeze_backbone_epochs', type=int, default=10, help='epochs to freeze the backbone in CoRA staged training.')
    parser.add_argument('--unfreeze_decoder_epochs', type=int, default=10, help='epochs to train decoder-only after the adapter warmup.')
    parser.add_argument('--backbone_lr_scale', type=float, default=0.1, help='learning-rate multiplier for the backbone during final CoRA finetuning.')
    ...
    apply_cora_link_prediction_configs(args=args)

    return args


def apply_cora_link_prediction_configs(args: argparse.Namespace):
    if args.decoder_variant != 'cora_lha':
        return

    if args.uv_cache_path is None:
        if args.dataset_name == 'USLegis':
            args.uv_cache_path = './processed_data/USLegis/us_projection_cache.npz'
        else:
            args.uv_cache_path = f'./processed_data/{args.dataset_name}/uv_projection_cache.npz'

    if not args.load_best_configs:
        return

    args.cora_num_slots = 8
    args.cora_profile_rank = 16
    args.cora_regime_dim = 32
    args.cora_adapter_hidden_dim = 172
    args.cora_symmetric_pairs = 1
    args.cora_enable_coalition = 1
    args.cora_enable_regime = 1
    args.freeze_backbone_epochs = 10
    args.unfreeze_decoder_epochs = 10
    args.backbone_lr_scale = 0.1

    if args.dataset_name == 'UNvote':
        args.uv_cache_path = './processed_data/UNvote/uv_projection_cache.npz'
    elif args.dataset_name == 'USLegis':
        args.uv_cache_path = './processed_data/USLegis/us_projection_cache.npz'
        args.gamma = 0.9
