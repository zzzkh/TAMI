        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.use_halt:
                    ...
                    num_negatives = halt.neg_sampler.num_negatives
                    negative_logits = negative_logits_flat.view(len(batch_src_node_ids), num_negatives)
                    tau = halt.compute_temperature(batch_src_node_ids, batch_node_interact_times).to(positive_logits.dtype)
                    loss = halt.listwise_loss(positive_logits=positive_logits, negative_logits=negative_logits, temperature=tau)

                    positive_probabilities = positive_logits.sigmoid()
                    negative_probabilities = negative_logits_flat.sigmoid()
                else:
                    ...
                    negative_probabilities = model[1](
                                                      ...
                                                      dst_ids=batch_neg_dst_node_ids,
                                                      src_emb=batch_neg_src_node_embeddings,
                                                      dst_emb=batch_neg_dst_node_embeddings).squeeze(dim=-1).sigmoid()

                predicts = torch.cat([positive_probabilities, negative_probabilities], dim=0)
                labels = torch.cat([torch.ones_like(positive_probabilities), torch.zeros_like(negative_probabilities)], dim=0)

                if not args.use_halt:
